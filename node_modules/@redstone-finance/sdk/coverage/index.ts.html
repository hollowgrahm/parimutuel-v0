...
&nbsp;
export const getDataServiceIdForSigner = <span class="fstat-no" title="function not covered" >(</span>
  oracleState: RedstoneOraclesState,
  signerAddress: string
) =&gt; {
<span class="cstat-no" title="statement not covered" >  for (const nodeDetails of Object.values(oracleState.nodes)) {</span>
<span class="cstat-no" title="statement not covered" >    <span class="missing-if-branch" title="if path not taken" >I</span>if (nodeDetails.evmAddress.toLowerCase() === signerAddress.toLowerCase()) {</span>
<span class="cstat-no" title="statement not covered" >      return nodeDetails.dataServiceId;</span>
    }
  }
<span class="cstat-no" title="statement not covered" >  throw new Error(`Data service not found for ${signerAddress}`);</span>
};
&nbsp;
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (reqParams.historicalTimestamp) {
<span class="cstat-no" title="statement not covered" >      return await Promise.any(promises);</span>
    }
&nbsp;
...
const prepareDataPackagePromises = (
  ...
  const pathComponents = [
    "data-packages",
    reqParams.historicalTimestamp ? <span class="branch-0 cbranch-no" title="branch not covered" >"historical" </span>: "latest",
    reqParams.dataServiceId,
  ];
  <span class="missing-if-branch" title="if path not taken" >I</span>if (reqParams.historicalTimestamp) {
<span class="cstat-no" title="statement not covered" >    pathComponents.push(`${reqParams.historicalTimestamp}`);</span>
  }
&nbsp;
  return urls.map((url) =&gt;
    axios
...
&nbsp;
  const requestedDataFeedIds = reqParams.dataFeeds ?? <span class="branch-1 cbranch-no" title="branch not covered" >[consts.ALL_FEEDS_KEY];</span>
&nbsp;
...
&nbsp;
export const getDecimalsForDataFeedId = <span class="fstat-no" title="function not covered" >(</span>
  dataPackages: SignedDataPackagePlainObj[]
) =&gt; {
  const firstDecimal = (<span class="cstat-no" title="statement not covered" >dataPackages[0].dataPoints[0] as INumericDataPoint)</span>
    .decimals;
  const areAllDecimalsEqual = <span class="cstat-no" title="statement not covered" >dataPackages.every(<span class="fstat-no" title="function not covered" >(d</span>ataPackage) =&gt;</span>
<span class="cstat-no" title="statement not covered" >    dataPackage.dataPoints.every(</span>
<span class="fstat-no" title="function not covered" >      (d</span>ataPoint) =&gt; (<span class="cstat-no" title="statement not covered" >dataPoint as INumericDataPoint).decimals === firstDecimal</span>
    )
  );
&nbsp;
<span class="cstat-no" title="statement not covered" >  <span class="missing-if-branch" title="if path not taken" >I</span>if (!areAllDecimalsEqual) {</span>
<span class="cstat-no" title="statement not covered" >    throw new Error("Decimals from data points in data packages are not equal");</span>
  }
<span class="cstat-no" title="statement not covered" >  return firstDecimal;</span>
};
&nbsp;
